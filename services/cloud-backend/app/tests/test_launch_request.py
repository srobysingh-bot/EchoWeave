...
def test_launch_and_play_route_to_registered_connector_with_logs(capfd):
    ...
    assert launch_resp.json()["response"]["outputSpeech"]["text"] == "Welcome to EchoWeave."
    ...
    assert "connector_dispatch_result connector_id=connector-e2e-1 success=True note=launch-routed" in log_output
    assert "connector_dispatch_result connector_id=connector-e2e-1 success=True note=play-routed" in log_output
