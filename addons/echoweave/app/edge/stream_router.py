...
async def edge_stream(queue_id: str, queue_item_id: str, request: Request):
    ...
    # If all candidates failed (404 from MA), the cached source URL may be stale.
    # Clear the cache and attempt a fresh resolution from MA (which may enqueue the
    # item or discover a different stream URL).
